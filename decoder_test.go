...
var unmarshalTests = []unmarshalTest{
	...
	{in: `{"Y": 1, "Z": 2}`, ptr: new(T), err: fmt.Errorf("sonnet: unknown field \"Z\""), disallowUnknownFields: true},

	{in: `{"alpha": "abc", "alphabet": "xyz"}`, ptr: new(U), out: U{Alphabet: "abc"}},
	{in: `{"alpha": "abc", "alphabet": "xyz"}`, ptr: new(U), err: fmt.Errorf("sonnet: unknown field \"alphabet\""), disallowUnknownFields: true},
	...
	},
	{
		in:                    `{"X": 1,"Y":2}`,
		ptr:                   new(S5),
		err:                   fmt.Errorf("sonnet: unknown field \"X\""),
	...
	{in: `{"B": "maybe"}`, ptr: new(B), err: errors.New(`sonnet: invalid use of ,string struct tag, trying to unmarshal "maybe" into bool`)},
	{in: `{"B": "tru"}`, ptr: new(B), err: errors.New(`sonnet: invalid use of ,string struct tag, trying to unmarshal "tru" into bool`)},
	{in: `{"B": "False"}`, ptr: new(B), err: errors.New(`sonnet: invalid use of ,string struct tag, trying to unmarshal "False" into bool`)},
	{in: `{"B": "null"}`, ptr: new(B), out: B{false}},
	{in: `{"B": "nul"}`, ptr: new(B), err: errors.New(`sonnet: invalid use of ,string struct tag, trying to unmarshal "nul" into bool`)},
	{in: `{"B": [2, 3]}`, ptr: new(B), err: errors.New(`sonnet: invalid use of ,string struct tag, trying to unmarshal unquoted value into bool`)},
	...
	// additional tests for disallowUnknownFields
	{
		...
		ptr:                   new(Top),
		err:                   fmt.Errorf("sonnet: unknown field \"extra\""),
		disallowUnknownFields: true,
	},
	{
		in: `{
	...
	},
	{
		in:  `"invalid"`,
		ptr: new(Number),
		err: fmt.Errorf("sonnet: invalid number literal, trying to unmarshal %q into Number", `"invalid"`),
	},
	{
		in:  `{"A":"invalid"}`,
...
func TestUnmarshalEmbeddedUnexported(t *testing.T) {
	...
	tests := []struct {
		...
		ptr: new(S1),
		out: &S1{R: 2},
		err: fmt.Errorf("sonnet: cannot set embedded pointer to unexported struct: sonnet.embed1"),
	}, {
